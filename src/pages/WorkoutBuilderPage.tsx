...
export default function WorkoutBuilderPage() {
  ...
  const [availableBlocks, setAvailableBlocks] = useState<Array<{ id: string; name: string; color: string }>>([]);
  const [orderedBlocks, setOrderedBlocks] = useState<BlockInstance[]>([]);
  ...
  const initializeBlocks = () => {
    const defaultBlocks = [
      { id: '1', name: language === 'es' ? 'Movilidad' : 'Mobility', color: 'green' },
      { id: '2', name: language === 'es' ? 'Entrada en Calor' : 'Warm-up', color: 'green' },
      { id: '3', name: language === 'es' ? 'Evaluación Rápida' : 'Pre-session Check', color: 'green' },
      { id: '4', name: language === 'es' ? 'Parte Principal' : 'Main Work', color: 'blue' },
      { id: '5', name: language === 'es' ? 'Trabajo Secundario' : 'Secondary Work', color: 'blue' },
      { id: '6', name: language === 'es' ? 'Superseries / Circuitos' : 'Superset / Complex', color: 'blue' },
      { id: '7', name: language === 'es' ? 'Acondicionamiento' : 'Conditioning', color: 'blue' },
      { id: '8', name: language === 'es' ? 'Accesorios / Preventivo' : 'Auxiliary / Prehab', color: 'orange' },
      { id: '9', name: language === 'es' ? 'Vuelta a la Calma' : 'Cool Down', color: 'orange' },
    ];
            ...
            {addPanelTab === 'circuit' ? (
              <div className="p-6">
                <CircuitPanelInline
                  language={language}
                  exercises={exercises}
                  sections={orderedBlocks.map(b => ({ id: b.instanceId, title: b.name, color: b.color, emoji: '', isEditing: false }))}
                  ...
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    {(() => {
                      const count = exercises.filter(ex => {
                        const search = bilingualExerciseMatch(ex, exerciseSearchTerm, language);
                        ...
                        return search.matched && matchesCategory && matchesType && matchesEquipment;
                      }).length;
                      return language === 'es'
                        ? `${count} ${count === 1 ? 'ejercicio encontrado' : 'ejercicios encontrados'}`
                        : `${count} ${count === 1 ? 'exercise found' : 'exercises found'}`;
                    })()}
                  </p>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {exercises
                      ...
                      .map(ex => {
                        const search = bilingualExerciseMatch(ex, exerciseSearchTerm, language);
                        const displayLang = exerciseSearchTerm.trim() ? search.matchedLanguage : language;
                        return (
                          <option key={ex.id} value={ex.id}>{getExerciseNameForLanguage(ex, displayLang)}</option>
                        );
                      })}
                  </select>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {orderedBlocks.map(block => (
                      <option key={block.instanceId} value={block.instanceId}>
                        {orderedBlocks.filter(b => b.typeId === block.typeId).length > 1
                          ? `${block.name} #${orderedBlocks.filter(b => b.typeId === block.typeId).findIndex(b => b.instanceId === block.instanceId) + 1}`
                          : block.name
                        }
                      </option>
                    ))}
          ...
          {(workoutExercises.length > 0 || workoutCircuits.length > 0) && (
            <div className="space-y-6">
              ...
              {(() => {
                ...
                // Order sections according to orderedBlocks (supports duplicates)
                ...
                return orderedSections.map(({ title: sectionTitle, items, color, instanceId }) => {
                  ...
                  return (
                    <div key={instanceId} className={`border-2 rounded-xl p-4 ${colorClass}`}>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {language === 'es'
                ? 'Arrastra para reordenar. Puedes agregar el mismo bloque varias veces.'
                : 'Drag to reorder. You can add the same block multiple times.'}
            </p>
            <BlockOrderBuilder
              availableBlocks={availableBlocks}
              orderedBlocks={orderedBlocks}
              onChange={setOrderedBlocks}
              language={language}
            />
          </div>
          ...
          <WorkoutSummaryPanel
            ...
            sections={orderedBlocks.map(b => ({ id: b.instanceId, title: b.name }))}
            language={language}
          />
