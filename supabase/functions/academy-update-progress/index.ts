...
const ACADEMY_URL = "https://xaatkjdbtlptbkdqbmih.supabase.co/functions/v1/hub-update-lesson-progress";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.lesson_id) {
      return new Response(JSON.stringify({ error: "lesson_id required in body" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    if (!body.user_id && !body.athlete_email) {
      body.athlete_email = user.email || "";
    }
