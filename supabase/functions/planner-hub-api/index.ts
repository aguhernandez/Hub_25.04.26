  ...
  try {
    ...
    if (endpoint === "push-race-plan" && req.method === "POST") {
      ...
      const {
        ...
        cho_loading_days,
        pre_comp_days,
        race_breakfast_timing,
        race_breakfast_description,
        race_breakfast_carbs_g,
        gi_training_weeks,
        gi_training_target_g_per_hour,
        gi_training_notes,
        gi_sessions,
        risks,
        athlete_notes,
      ...
      const { data: newPlan, error: insertError } = await supabaseAdmin
        ...
        .insert({
          ...
          sweat_rate_l_per_hour: sweat_rate_l_per_hour ?? null,
          projected_mass_loss_pct: projected_mass_loss_pct ?? null,
          caffeine_total_mg: caffeine_total_mg ?? null,
          caffeine_mg_per_kg: caffeine_mg_per_kg ?? null,
          ...
          caffeine_sources: caffeine_sources ?? null,
          caffeine_notes: caffeine_notes ?? null,
          segments: segments ?? null,
          pre_comp_notes: pre_comp_notes ?? null,
          cho_loading_days: cho_loading_days ?? null,
          pre_comp_days: pre_comp_days ?? null,
          race_breakfast_timing: race_breakfast_timing ?? null,
          race_breakfast_description: race_breakfast_description ?? null,
          race_breakfast_carbs_g: race_breakfast_carbs_g ?? null,
          gi_training_weeks: gi_training_weeks ?? null,
          gi_training_target_g_per_hour: gi_training_target_g_per_hour ?? null,
          gi_training_notes: gi_training_notes ?? null,
          gi_sessions: gi_sessions ?? null,
          risks: risks ?? null,
        ...
        .select("id, race_name, sport, race_date, scheduled_date, expected_duration_min")
        .single();
    ...
    // ──────────────────────────────────────────────────────────────────
    // GET /planner-hub-api/activities
    // Returns GPS/Strava activities (external_activities) for an athlete
    // Query params: athlete_email or athlete_id, date_from, date_to, limit
    // ──────────────────────────────────────────────────────────────────
    if (endpoint === "activities" && req.method === "GET") {
      const dateFrom = url.searchParams.get("date_from") || new Date(Date.now() - 30 * 86400000).toISOString().split("T")[0];
      const dateTo = url.searchParams.get("date_to") || new Date().toISOString().split("T")[0];
      const limit = Math.min(parseInt(url.searchParams.get("limit") || "50"), 200);
      ...
      const { data: activities, error: actError } = await supabaseAdmin
        ...
        `)
        .eq("user_id", athleteId)
        .gte("local_date", dateFrom)
        .lte("local_date", dateTo)
        .is("deleted_at", null)
