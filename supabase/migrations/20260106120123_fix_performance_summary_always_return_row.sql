/*
  # Fix Performance Summary to Always Return a Row

  ## Problem
  Previous version returns NO ROWS if performance_sessions is empty,
  causing "No performance data" error even when external_activities 
  and extra_training_logs exist.

  ## Solution
  - Always return exactly one row with aggregated data
  - Calculate totals from all three sources independently
  - Use direct SELECT instead of FROM to guarantee a row

  ## Impact
  - Athletes will see their training data even without performance_sessions
  - External activities and extra training logs will show correctly
*/
...
DECLARE
  ...
  v_avg_session_volume numeric;
  v_avg_session_rpe numeric;
BEGIN
  -- Get ACWR data
  ...
  SELECT 
    ...
    COALESCE(AVG(total_volume), 0),
    COALESCE(AVG(session_rpe), 0)
  INTO 
    v_performance_sessions_count,
    v_total_volume,
    v_avg_session_volume,
    v_avg_session_rpe
