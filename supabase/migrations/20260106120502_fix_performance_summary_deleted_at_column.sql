/*
  # Fix Performance Summary to Use deleted_at Instead of is_deleted

  ## Problem
  Function references non-existent column is_deleted in external_activities.
  The table actually uses deleted_at (soft delete pattern).

  ## Solution
  - Change is_deleted = false to deleted_at IS NULL
  - This correctly filters out soft-deleted external activities

  ## Impact
  - Function will execute successfully
  - Only non-deleted external activities will be counted
*/
...
BEGIN
  ...
  -- Count external activities (using deleted_at IS NULL instead of is_deleted = false)
