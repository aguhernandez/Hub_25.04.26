/*
  ...
  2. Security
    ...
    - Trainers can view and manage logs for their assigned athletes
...
-- Trainers can view logs of their assigned athletes
CREATE POLICY "Trainers can view assigned athletes extra logs"
  ON extra_training_logs
  FOR SELECT
  TO authenticated
  USING (
    ...
    AND (
      athlete_id IN (
        SELECT athlete_id FROM trainer_athlete_assignments
...
-- Admins can view all logs
CREATE POLICY "Admins can view all extra training logs"
  ON extra_training_logs
  FOR SELECT
  TO authenticated
  USING (
